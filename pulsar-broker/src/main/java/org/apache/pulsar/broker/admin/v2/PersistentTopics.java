...
public class PersistentTopics extends PersistentTopicsBase {
    ...
    public void truncateTopic(
            ...
            @ApiParam(value = "Specify topic name", required = true)
            @PathParam("topic") @Encoded String encodedTopic,
            @ApiParam(value = "Whether the retention Constraint is ignored",
                    defaultValue = "false", type = "boolean")
            @QueryParam("skipRetentionConstraint") @DefaultValue("false") boolean skipRetentionConstraint){
            validateTopicName(tenant, namespace, encodedTopic);
            internalTruncateTopic(skipRetentionConstraint);
